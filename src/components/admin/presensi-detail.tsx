...
export function PresensiDetail({ type }: { type: 'event' | 'activity' }) {
  const params = useParams();
  const plural = type === 'event' ? 'events' : 'activities';
  ...
  const fetchData = (currentTab = tab, currentSearch = search) => {
    fetch(`/api/admin/registrants?type=${type}&id=${params.id}&tab=${currentTab}&search=${currentSearch}`)
      .then((r) => r.json())
      .then((d) => {
  ...
  const toggleHadir = async (registrationId: number) => {
    const res = await fetch('/api/admin/presensi', {
      ...
      body: JSON.stringify({ action: 'toggle-hadir', type, id: params.id, registrationId }),
    });
  ...
  const toggleScan = async () => {
    await fetch('/api/admin/presensi', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'toggle-scan', type, id: params.id }),
    });
    fetchData();
  };
  ...
  const updateScanPin = async () => {
    ...
    await fetch('/api/admin/presensi', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'update-scan-pin', type, id: params.id, scan_pin: scanPin }),
    });
    setScanPin('');
  ...
  const quickMark = async () => {
    ...
    const res = await fetch('/api/admin/presensi', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'quick-mark', type, id: params.id, code: quickCode.trim() }),
    });
    const data = await res.json();
    setQuickResult(data);
    setQuickCode('');
  ...
  if (!item) return <Loading label="Memuat data presensi..." />;
        ...
        <div className="rounded-xl border border-border/60 bg-card p-6 shadow-card">
          ...
          <div className="mt-4 space-y-4">
            <div className="flex items-center gap-3">
              <Button
                onClick={toggleScan}
                variant={item.scan_active ? 'destructive' : 'default'}
              >
                {item.scan_active ? 'Nonaktifkan Scan' : 'Aktifkan Scan'}
              </Button>
              {item.scan_active && (
            ...
            {item.scan_pin && <p className="text-xs text-muted-foreground">PIN saat ini: {item.scan_pin}</p>}
          </div>
        </div>
