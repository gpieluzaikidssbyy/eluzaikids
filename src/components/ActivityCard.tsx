    ...
    <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
          ...
          <div className="space-y-1.5 text-sm">
            {activity.tema && (
              <p className="text-slate-600 dark:text-slate-400">
                <span className="font-medium text-slate-900 dark:text-slate-100">Tema:</span> {activity.tema}
              </p>
            )}
            {activity.activity_date && (
              <p className="text-slate-600 dark:text-slate-400">
                <span className="font-medium text-slate-900 dark:text-slate-100">Tanggal:</span> {formatDateIndo(activity.activity_date)}
              </p>
            )}
            {activity.location && (
              <p className="text-slate-600 dark:text-slate-400">
                <span className="font-medium text-slate-900 dark:text-slate-100">Lokasi:</span> {activity.location}
              </p>
            )}
          </div>
        )}
          ...
          <div className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
            ...
            {activity.activity_date && (
              <p className="flex items-center gap-2">
                ...
                {formatDateIndo(activity.activity_date)}
              </p>
            )}
            {activity.location && (
              <p className="flex items-center gap-2">
        ...
        <div className="mt-auto flex gap-3 pt-3">
          ...
          {isOver ? (
            // Kegiatan selesai: tombol jadi link Google Drive foto (hijau = link tersedia).
            activity.drive_link ? (
              <a
