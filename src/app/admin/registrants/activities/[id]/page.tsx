...
import { ArrowLeft, Pencil, Trash2, Loader2, Users, ClipboardList } from 'lucide-react';
...
export default function AdminRegistrantActivityDetailPage() {
  ...
  const handleEdit = async () => {
    ...
    setEditing(true);
    const response = await fetch(`/api/admin/registrants?type=activity&id=${editTarget.id}`, {
      method: 'PATCH',
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    const response = await fetch(`/api/admin/registrants?type=activity&id=${deleteTarget.id}`, {
      method: 'DELETE',
    });
    const data = await response.json();
    setDeleting(false);
  ...
  if (!activity) return <Loading label="Memuat data pendaftar..." />;

  const totalPeserta = registrations.reduce((sum: number, r: any) => sum + (r.jumlah_hadir || 0), 0);
  ...
  return (
    ...
    >
      <PageHeader
        ...
        title={`Manage Registrants: ${activity?.title || '...'}`}
        backHref="/admin/registrants/activities"
      />

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <StatCard label="Total Pendaftar" value={registrations.length} icon={ClipboardList} gradient="from-amber-500 to-yellow-600" index={0} />
      ...
      >
        <div className="overflow-x-auto rounded-lg border border-border bg-card shadow-sm">
          ...
          {!registrations.length && (
            <EmptyState icon={ArrowLeft} title="Belum ada pendaftar" description="Belum ada pendaftar untuk kegiatan ini." className="border-t" />
          )}
        </div>
      </motion.div>
      ...
      <AlertDialog open={!!editTarget} onOpenChange={(open) => !open && setEditTarget(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction type="submit" form="edit-registrant-form" disabled={editing}>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) { setDeleteTarget(null); setDeleteConfirm(''); } }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tindakan ini tidak dapat dibatalkan. Pendaftar <span className="font-semibold text-foreground">{deleteTarget?.name}</span> akan dihapus dari daftar pendaftaran kegiatan ini.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="space-y-1.5">
