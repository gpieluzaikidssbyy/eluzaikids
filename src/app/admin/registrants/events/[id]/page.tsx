...
export default function AdminRegistrantEventDetailPage() {
  ...
  const [editTarget, setEditTarget] = useState<EventRegistration | null>(null);
  ...
  const handleEdit = async () => {
    ...
    setEditing(true);
    const response = await fetch(`/api/admin/registrants?type=event&id=${editTarget.id}`, {
      method: 'PATCH',
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    const response = await fetch(`/api/admin/registrants?type=event&id=${deleteTarget.id}`, {
      method: 'DELETE',
    });
    const data = await response.json();
    setDeleting(false);
  ...
  if (!event) return <Loading label="Memuat data pendaftar..." />;
  ...
  return (
    ...
    >
      <PageHeader
        ...
        title={`Manage Registrants: ${event?.title || '...'}`}
        description={event?.tema ? `Tema: ${event.tema}` : undefined}
        backHref="/admin/registrants/events"
        actions={
          <a
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <StatCard label="Total Pendaftar" value={registrations.length} icon={ClipboardList} gradient="from-primary to-blue-700" index={0} />
      ...
      >
        <div className="overflow-x-auto rounded-lg border border-border bg-card shadow-sm">
          ...
          {!registrations.length && (
            <EmptyState icon={ArrowLeft} title="Belum ada pendaftar" description="Belum ada pendaftar untuk event ini." className="border-t" />
          )}
        </div>
      </motion.div>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) { setDeleteTarget(null); setDeleteConfirm(''); } }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tindakan ini tidak dapat dibatalkan. Pendaftar <span className="font-semibold text-foreground">{deleteTarget?.name}</span> akan dihapus dari daftar pendaftaran event ini.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="space-y-1.5">
