...
export default function AdminRegistrantEventsPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500">Pilih event untuk melihat daftar pendaftarnya.</p>
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {events.map((event) => (
            <Link
              ...
              className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_8px_30px_rgba(15,23,42,0.05)] transition duration-200 hover:-translate-y-1 hover:border-brand-300 hover:shadow-[0_16px_36px_rgba(15,23,42,0.1)] dark:border-slate-800 dark:bg-slate-900"
            >
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="mt-1 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-brand-600 dark:bg-brand-950/40 dark:text-brand-300">
                  <svg className="h-4.5 w-4.5" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round">
                    <rect x="3" y="4" width="18" height="17" rx="2" />
                    <path d="M16 2v4M8 2v4M3 10h18" />
                  </svg>
                </span>
              </div>
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{formatDateIndo(event.event_date)}</p>
              {event.location && <p className="mt-1 truncate text-xs text-slate-400">{event.location}</p>}
              <div className="mt-4 flex items-center justify-between">
                <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold ${event.registrations_count > 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400'}`}>
                ...
                <span className="text-slate-300 transition group-hover:translate-x-1 group-hover:text-brand-500 dark:text-slate-600" aria-hidden="true">
