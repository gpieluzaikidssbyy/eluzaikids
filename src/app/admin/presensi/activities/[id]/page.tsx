...
export default function AdminPresensiActivityDetailPage() {
  ...
  if (!activity) return <Loading label="Memuat data presensi..." />;
  ...
  return (
    ...
    >
      <PageHeader
        ...
        title={`Presensi: ${activity.title}`}
        description={activity.location || ''}
        backHref="/admin/presensi/activities"
      />
        ...
        <div className="rounded-xl border border-border/60 bg-card p-6 shadow-card">
          ...
          <div className="mt-4 space-y-4">
            <div className="flex items-center gap-3">
              <Button
                onClick={toggleScan}
                variant={activity.scan_active ? 'destructive' : 'default'}
              >
                {activity.scan_active ? 'Nonaktifkan Scan' : 'Aktifkan Scan'}
              </Button>
              {activity.scan_active && (
            ...
            {activity.scan_pin && <p className="text-xs text-muted-foreground">PIN saat ini: {activity.scan_pin}</p>}
          </div>
        </div>
